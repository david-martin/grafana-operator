...
COMPILE_OUTPUT = build/_output/bin/grafana-operator
...
test/e2e:
	...
	operator-sdk test local ./test/e2e --go-test-flags "-v" --kubeconfig "${HOME}/.kube/config"

.PHONY: test/e2e-up-local
test/e2e-up-local:
	@echo Running e2e tests with local operator:
	operator-sdk test local ./test/e2e --go-test-flags "-v" --kubeconfig "${HOME}/.kube/config" --up-local --namespace=${NAMESPACE}
